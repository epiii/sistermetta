...
echo "<html><head><title>Nota Transaksi Penjualan Jasa</title>";
...
echo'</td></tr><tr><td>';
$no=1;
$query 		= mysql_query ("SELECT * FROM `pos_penjualanjasa` WHERE `nofaktur` like '$kode'");
...
echo '</table>';	
echo '<b>Detail Transaksi Penjualan Jasa</b>';	
echo '
<table>';
echo '	
<tr>
...
$hasild = $koneksi_db->sql_query("SELECT * FROM `pos_penjualanjasadetail` WHERE `nofaktur` like '$kode'");
while ($datad =  $koneksi_db->sql_fetchrow ($hasild)){
echo '	
<tr>
<td class="border">'.$no.'</td>
<td class="border">'.getjenis($datad["jenis"]).'</td>
<td class="border">'.$datad["kodejasa"].'</td>
<td class="border">'.getnamajasa($datad["kodejasa"]).'</td>

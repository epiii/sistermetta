<?php
	...
	if(!isset($_POST['aksi'])){
		...
		switch ($_POST['aksi']) {
			// -----------------------------------------------------------------
			case 'tampil':
				$golongan   = isset($_POST['golonganS'])?filter(trim($_POST['golonganS'])):'';
				$keterangan = isset($_POST['keteranganS'])?filter(trim($_POST['keteranganS'])):'';
				$sql = 'SELECT *
						FROM '.$tb.'
						WHERE 
							golongan like "%'.$golongan.'%" and
				...
				if($jum!=0){	
					...
					while($res = mysql_fetch_assoc($result)){	
						// urutan
							$nox = '<select '.(isAksi('golongan','u')?'onchange="urutFC(this);"':'disabled').' class="text-center" replid1="'.$res['replid'].'" urutan1="'.$res['urutan'].'" >';
							for($i=1; $i<=$jum; $i++){
						...
						$btn ='<td>
									<button '.(isAksi('golongan','u')?'onclick="viewFR('.$res['replid'].');"':'disabled').' data-hint="ubah"  >
										<i class="icon-pencil on-left"></i>
									</button>
									<button  '.(isAksi('golongan','d')?'onclick="del('.$res['replid'].');"':'disabled').' data-hint="hapus" onclick="del('.$res['replid'].');">

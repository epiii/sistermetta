...
// combo kelompok ---
    function cmbkelompok(thn,kel){
        var u = dir_kel;
        var d = 'aksi=cmbkelompok&'+(thn!=''?'proses='+thn:'replid='+kel);
        ajax(u,d).done(function (dt){
            var out='';
            if(dt.status!='sukses'){
                out+='<option value="">'+dt.status+'</option>';
            ...
            if(kel!='') {
                ...
                $('#kelompokTD').html(dt.kelompok[0].kelompok);
            }else {
                $('#kelompokTD').html(dt.kelompok[0].kelompok);
                console.log('masuk filtering');
                $('#kelompokS').html(out);
                viewTB(); 
            }
        });

$(document).ready(function() {
	...
	var UserEnterMessage = "Silahkan Masukkan username dan password";
	...
	$(".loginMessage").html(UserEnterMessage);
	...
	// content += '<div class="loginFail"><span id="errorMessage" class="OrangeSpan">'+ ErrorMessage +'</span><br/><br/><button id="botTryAgain">'+ ErrorBotonLabel +'</button></div>';
...
}); //end of rady function


// login function
function loginFC(){
	var u = 'lib/dblogin.php';
	var d = $(this).serialize();
	ajax(u,d).done(function (dt){
		if(!dt.stat){ // fail login
			errorMessage(dt.msg);
...
function errorMessage (erMsg) {
	OnError = true;
	content ='<div class="boxLoading"><img src="img/loading82.gif"><span>'+LoadingMessage+'</span></div>'
			+'<div class="loginFail"><span id="errorMessage" class="OrangeSpan">'+ erMsg +'</span><br/><br/><button id="botTryAgain">'+ ErrorBotonLabel +'</button></div>';
	$(".LoginBox").append(content);
	if(OnError){
		$("#botTryAgain").click();
		return false;

	...
	if(!isset($_POST['aksi'])){
		...
		switch ($_POST['aksi']) {
			...
			case 'tampil':
				...
				$katgrupmenu = (isset($_POST['katgrupmenuS']) and trim($_POST['katgrupmenuS'])!='')?filter($_POST['katgrupmenuS']):'';
				$sql  = 'SELECT *
						FROM '.$tb.'
						WHERE 
							'.$katgrupmenu.' like "%'.$katgrupmenu.'%"  AND
				...
				if($jum!=0){	
					...
					while($res = mysql_fetch_assoc($result)){	
						...
						$out.= '<tr>
									...
									<td align="center">'.$res['katgrupmenu'].'</td>

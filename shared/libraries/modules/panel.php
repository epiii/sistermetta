...
function panel_tile2($t,$p,$x=0,$y=0,$i=0){
	global $tilecount; $act=str_replace('std','openPage('.$t.',\''.$p['key'].'\',true)',$p['action']);
	echo '<div class="tilebox" style="width:288px;left:'.($x*288).'px;top:'.($y*143).'px">';
	echo '<a id="tile_'.$t.'_'.$i.'" href="#&'.$p['key'].'" onclick="'.$act.'" class="tile" style="border-radius:6px !important;background:url(\''.(IMGR.$p['icon']).'\') 20px 22px no-repeat '.$p['color'].';width:240px;height:110px"><div style="margin-left:'.($p['icon']==''?'0':'40').'px">';
	echo '<div class="tiletitle">'.$p['title'].'</div>';
...
function panel_tile5($t,$p,$x=0,$y=0,$i=0){
	...
	echo '<a id="tile_'.$t.'_'.$i.'" href="#&'.$p['key'].'" onclick="'.$act.'" class="tile" style="border-radius:6px !important;background:'.$p['color'].';width:'.($tw-48).'px;height:110px">';
	echo '<div class="tiletitle">'.$p['title'].'</div>';
	if($p['desc']!='') echo '<div class="tiledesc">'.$p['desc'].'</div>';
	echo '</a></div>';
}
...
function panel_draw(){
	...
	for($t=0;$t<$n1;$t++){
		...
		for($i=0;$i<$n2;$i++){
			...
			if($tip==2) panel_tile2($t,$pages[$i],$x,$y,$i);
			else if($tip==3) panel_tile3($t,$pages[$i],$x,$y,$i);
			else if($tip==4) panel_tile4($t,$pages[$i],$x,$y,$i);
			else if($tip==5||$tip==6) panel_tile5($t,$pages[$i],$x,$y,$i);
			else if($tip==7) panel_tile7($t,$pages[$i],$x,$y,$i);
			else if($tip==8) panel_tile8($t,$pages[$i],$x,$y,$i);
			else if($tip==100) panel_tile100($t,$pages[$i],$x,$y,$i);
			else panel_tile($t,$pages[$i],$x,$y,$i);
			$x++;
		}
		echo '</div>';
